...
bl_info = {
    ...
    "author": "Philipp Oeser,(Port to 2.80 by Sav Martin",
    "version": (0, 1),
    "blender": (2, 80, 0),
    "api": 42672,
...
def licGetClosestPoint(oObject, vPoint):
    ...
    oDefMesh = oObject.to_mesh(depsgraph=bpy.context.depsgraph, apply_modifiers=True)
...
class DATA_PT_lichtwerk_meshtransfer(bpy.types.Panel):
    ...
    # bl_idname = "DATA_PT_lichtwerk_meshtransfer"
    ...
    def draw(self, context):
        ...
        if oTargetObject.data is not None and oTargetObject.data.bl_rna.identifier == "Mesh":
            ...
            row.label(text=' %s is mapped to: %s' % (oTargetObject.name, ', '.join(lMappings)), icon='INFO')
            if len(lMappings) > 0:
                bActiveOps = True
            row = box.row()
            ...
            if oWM.lMeshTransfer_Operation == '0':
                ...
                row = box.row()
                row.label(text=' experimental feature', icon='ERROR')
    ...
    def execute(self, context):
        ...
        for oSourceVertexGroup in lSourceVertexGroupsToTransfer:
            ...
            if sTargetVertexGroup in oTargetObject.vertex_groups:
                ...
                oTargetVertexGroup = oTargetObject.vertex_groups.new(name=sTargetVertexGroup)
...
class DATA_OT_lichtwerk_meshtransfer_shapekeys(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if oTargetObject.data.shape_keys is None:
            oTargetReferenceKey = oTargetObject.shape_key_add(name="MeshTransfer [Basis]")
        ...
        for oSourceShapekey in lSourceShapekeysToTransfer:
            ...
            if sTargetShapekey in [o.name for o in oTargetObject.data.shape_keys.key_blocks]:
                ...
                oTargetShapekey = oTargetObject.shape_key_add(name=sTargetShapekey)
                bNeedToCalcOrigDisp = False
...
class DATA_OT_lichtwerk_meshtransfer_shape(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        oKey_start = oTargetObject.shape_key_add(name="MeshTransfer [%s] start" % sSourceObject)
        oKey_deform = oTargetObject.shape_key_add(name="MeshTransfer [%s] deform" % sSourceObject)
...
classes = ( DATA_PT_lichtwerk_meshtransfer,
